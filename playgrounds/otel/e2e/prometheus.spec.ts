...
test.describe("Astro Prometheus Integration", () => {
	...
	const metricsURL = "http://localhost:8080/prometheus";
	...
	test.describe("Metrics Endpoint", () => {
		...
		test("should return Prometheus text format", async ({ page }) => {
			const response = await page.goto(`${metricsURL}`);
		...
		test("should include custom prefix in metric names", async ({ page }) => {
			...
			// Should have custom prefix "myapp__" on all custom metrics (OpenTelemetry uses double underscore)
			expect(content).toContain("myapp__http_requests_total");
			expect(content).toContain("myapp__http_request_duration_seconds");
			expect(content).toContain("myapp__http_server_duration_seconds");
		});
		...
		test("should include custom labels on metrics", async ({ page }) => {
			...
			// Should include standard OpenTelemetry labels that are actually present
		...
		test("should include default Node.js metrics with prefix", async ({
			...
			// Should include default metrics with custom prefix (actual OpenTelemetry metric names)
			expect(content).toContain("myapp__process_cpu_time_total");
			expect(content).toContain("myapp__process_memory_usage");
		});
	});
	...
	test.describe("Page Navigation and Metrics Collection", () => {
		...
		test("should collect duration metrics correctly", async ({ page }) => {
			...
			expect(content).toContain("myapp__http_request_duration_seconds_count");
			expect(content).toContain("myapp__http_server_duration_seconds_count");
		});
	});
	...
	test.describe("Performance and Caching", () => {
		...
		test("should maintain metric consistency across requests", async ({
			...
			expect(updatedContent).toContain("myapp__http_requests_total");
			expect(updatedContent).toContain("myapp__http_request_duration_seconds");
	...
	test.describe("Integration Configuration", () => {
		test("should respect custom metrics URL configuration", async ({
			...
			// The integration is configured with metricsUrl: "/prometheus"
		...
		test("should apply custom prefix to all metrics", async ({ page }) => {
			...
			// All custom metrics should have the "myapp__" prefix (OpenTelemetry uses double underscore)
			expect(content).toContain("myapp__http_requests_total");
			expect(content).toContain("myapp__http_request_duration_seconds");
			expect(content).toContain("myapp__http_server_duration_seconds");
		...
		test("should include custom labels on all metrics", async ({ page }) => {
			...
			// Standard OpenTelemetry labels should be present on metrics
			expect(content).toContain('method="GET"');
			expect(content).toContain('path="/"');
			expect(content).toContain('status="200"');
		});
