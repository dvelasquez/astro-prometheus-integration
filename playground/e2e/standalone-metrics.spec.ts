...
test.describe('Standalone Metrics Server', () => {
  ...
  test.skip('should start standalone metrics server when enabled', async ({ page }) => {
  ...
  test('should not expose metrics on standalone port when disabled', async ({ page }) => {
    ...
    try {
      ...
      console.log('Standalone metrics server is not running (expected behavior)');
    }
  });

  test('should still expose metrics on main app endpoint when standalone is disabled', async ({ page }) => {
