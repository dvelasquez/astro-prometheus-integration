...
test.describe('Astro Prometheus Integration', () => {
  ...
  test.describe('Metrics Endpoint', () => {
    ...
    test('should include default Node.js metrics with prefix', async ({ page }) => {
      await page.goto(`${baseURL}${metricsURL}`);
  ...
  test.describe('Performance and Caching', () => {
    test('should not call expensive operations on every request', async ({ page }) => {
      ...
      const requestCountMatch = content.match(/myapp_http_requests_total\{[^}]*\} (\d+)/);
      if (requestCountMatch) {
        const requestCount = parseInt(requestCountMatch[1]);
        expect(requestCount).toBeGreaterThan(0);
        expect(requestCount).toBeLessThan(100); // Reasonable upper bound
      }
    });

    test('should maintain metric consistency across requests', async ({ page }) => {
      // First visit
      await page.goto(`${baseURL}/`);
      await page.waitForTimeout(100);
      ...
      const initialMatch = initialContent.match(/myapp_http_requests_total\{[^}]*\} (\d+)/);
      const updatedMatch = updatedContent.match(/myapp_http_requests_total\{[^}]*\} (\d+)/);
  ...
  test.describe('Integration Configuration', () => {
    test('should respect custom metrics URL configuration', async ({ page }) => {
